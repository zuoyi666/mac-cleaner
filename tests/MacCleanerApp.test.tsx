...
describe('MacCleanerApp', () => {
  ...
  it('shows scan results and requires confirmation before cleanup', async () => {
    ...
    expect(screen.getAllByText('建议用原工具处理').length).toBeGreaterThan(0)
    expect(screen.getAllByText('别一键删').length).toBeGreaterThan(0)
    expect(screen.getByText('逐项审稿单')).toBeInTheDocument()
    expect(screen.getByText('我为什么这么判断')).toBeInTheDocument()
    expect(screen.getByText('这次会移动什么')).toBeInTheDocument()
    expect(screen.getByText('这次不会碰什么')).toBeInTheDocument()
    expect(screen.queryByRole('button', { name: `移到废纸篓: ${firstCandidate.title}` })).not.toBeInTheDocument()
    ...
    expect(screen.getAllByText('不会碰清单外路径').length).toBeGreaterThan(0)
    ...
    await user.click(screen.getByRole('button', { name: /我看懂了，只移到废纸篓/ }))
  ...
  it('can select multiple cleanable rows before opening one confirmation', async () => {
    ...
    expect(screen.getByRole('checkbox', { name: /Xcode DerivedData/ })).not.toBeDisabled()
    expect(screen.getByRole('checkbox', { name: /DesignTool-4.2.1.dmg/ })).toBeDisabled()
    expect(screen.queryByRole('button', { name: /移到废纸篓: Xcode DerivedData/ })).not.toBeInTheDocument()

    await user.click(screen.getByRole('button', { name: /选择安全项/ }))
    await user.click(screen.getByRole('button', { name: /打开最后确认/ }))
  ...
  it('switches the current UI to English without rescanning', async () => {
    ...
    expect(screen.getByText('Item review sheet')).toBeInTheDocument()
    expect(screen.getByText('Why I think this')).toBeInTheDocument()
    expect(screen.getByText('What would move')).toBeInTheDocument()
